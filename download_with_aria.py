...
if response.status_code == 200:
    ...
    try:
        # Use aria2c for much faster downloads
        exit_code = os.system(
            f'aria2c -x 8 -s 8 --file-allocation=none --continue=true --summary-interval=10 '
            f'--out="{filename}" '
            f'"https://civitai.com/api/download/models/{args.model}?type=Model&format=SafeTensor&token={token}"'
        )

        if exit_code != 0:
            print(f"Error: aria2c failed with exit code {exit_code}")
